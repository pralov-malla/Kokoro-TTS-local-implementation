...
def clean(text):
    """Remove punctuation and lowercase — so 'Hello,' matches 'hello'."""
    return re.sub(r"[^\w\s']", "", text.lower()).split()


def find_phrase(words, phrase):
    """Find phrase in JSON word list. Returns (start_index, end_index)."""
    tokens, source_index = [], []
...
def extract_clip(wav, jsn, phrase, pad_start_ms, pad_end_ms):
    """Cut a phrase out of a WAV using its JSON timestamps."""
...
sum(parts).export(DIR / FINAL_OUT, format="wav")
